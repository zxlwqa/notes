...
const handleGet: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    if (pathParts.length >= 3 && pathParts[2]) {
      ...
      const notes = results.map((note: NoteRow) => {
        let tags = [];
        try {
    ...
    await logToD1(env, 'error', 'notes.get.exception', { message: error instanceof Error ? error.message : String(error) })
...
const handlePost: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    console.error('Database error:', error);
    await logToD1(env, 'error', 'notes.create.exception', { message: error instanceof Error ? error.message : String(error) })
    return new Response(JSON.stringify({ error: "Internal server error" }), {
      status: 500,
      headers: {
        'Content-Type': 'application/json',
...
const handlePut: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    const existingNote = await env.NOTESD!.prepare(`SELECT id FROM notes WHERE id = ?`).bind(noteId).first();
    if (!existingNote) {
      return new Response(JSON.stringify({ error: "Note not found" }), {
        status: 404,
        headers: {
    ...
    console.error('Database error:', error);
    await logToD1(env, 'error', 'notes.update.exception', { message: error instanceof Error ? error.message : String(error) })
    return new Response(JSON.stringify({ error: "Internal server error" }), {
      status: 500,
      headers: {
        'Content-Type': 'application/json',
