...
export const onRequestPost: PagesFunction<{
  ...
  try {
    ...
    await logToD1(context.env, 'error', 'backup.upload.db_error', { message: e instanceof Error ? e.message : String(e) })
    return new Response(
  ...
  try {
    ...
    await logToD1(context.env, 'error', 'backup.upload.exception', { message: errorMessage })
    return new Response(
...
export const onRequestGet: PagesFunction<{
  ...
  try {
    ...
    const errorMessage = e instanceof Error ? e.message : String(e)
    await logToD1(context.env, 'error', 'backup.download.exception', { message: errorMessage })
    return new Response(
      JSON.stringify({ success: false, error: errorMessage }),
      { status: 500, headers: corsHeaders }
    );
